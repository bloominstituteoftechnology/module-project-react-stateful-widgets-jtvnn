...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    return programmers.find(program => program.id === selectedProgrammer).name
  };
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { /* in here set the featured id to be dev.id */setSelectedProgrammer(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
